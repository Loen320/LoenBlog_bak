...
<div id="login">
    ...
    <form name="loginForm" id="loginForm"  method="post">
        ...
        <p>
            <input type="text" name="image" placeholder="请输入验证码" id="verify_input"/>
            <a href="javascript:getVerifiCode()" id="verify_code">
                <img id="yzm_img" style="cursor:pointer;width: 100px;height: 37.4px;padding-left: 18px;border-radius: 3px;" title="点击刷新验证码" src="/getVerifiCode"/>
            </a>
        </p>
    ...
    <p id="backtoblog"><a href="/">&larr; 返回到Loen博客</a></p>
        ...
        if(user=="") {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.code==0) {
                        alert(data.msg);
                        $("#yzm_img").prop('src','/getVerifiCode?a='+new Date().getTime());//输入错误刷新验证码
                    } else {
                ...
                error: function () {
                    //alert("数据获取失败")
                    document.write(data.msg+"数据获取失败");
                    window.location.href="/admin";
                }
            })
        }
